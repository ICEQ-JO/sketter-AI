...
function systemPrompt(canvasSummary: CanvasSummary, mode: ChatMode): string {
  ...
  if (mode === "plan") {
    return [
      ...
      "Your job is to understand what diagram the user wants, then propose a clear plan. Start by asking 1–3 focused clarifying questions when anything is genuinely ambiguous (scope, shapes, relationships, labels, style). Prefer multiple-choice options when there's a natural set of choices.",
  ...
  return [
    ...
    "update_element can also change an existing element's shape (its `type`) — e.g. 'change all the boxes to circles' means calling update_element with type: 'ellipse' on each existing rectangle, not deleting and recreating them.",
